...
4. 看到应用列表后，亲手点磁贴「研究生综合管理」（门户 SSO）。不要自己打开 `https://wxt.zgysyjy.org.cn:7792/graduate/frameset.jsp`。
5. 磁贴失败或出现「请登录」：关闭，改用截图。应用不会再跳一次裸 frameset。
6. 亲手点磁贴进入后，点左侧 **我的课表** 等到周课表格子出现，再点右上角 **录入课表**。应用从 frameset **子页面**抽表格（不是只读外壳页）；弹出核对清单后，你点 **写入本机课表** 才会落盘。横幅可出现 `tables=N frames=M`。若还在应用列表就点录入，会提示先打开周视图，不会弹出空清单。相册多图 DeepSeek 仍在 **导入** 页。
